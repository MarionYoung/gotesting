...
func (f *FileLogger) splitFile(file *os.File) (*os.File, error) {
	...
	fileINfo, err := file.Stat()
	if err != nil {
		fmt.Printf("get log file info failed,err:%v\n", err)
		return nil, err
	}
	logName := path.Join(f.filePath, fileINfo.Name())      // 拿到当前的日志文件绝对路径
	newLogName := fmt.Sprintf("%s.bak%s", logName, nowStr) // 拼接日志文件备份名字
	// 1.关闭当前的日志文件
	file.Close()
	// 2. 备份一下 rename  xx.log  ->  xx.log.bak202007031538
